...
class DriverRechargeLogRepository @Inject constructor(val apiService: ApiService) {

    suspend fun getDriverRechargeLogs(requestDriverRechargeLogs: RequestDriverRechargeLogs): UiState<ResponseDriverWalletLogs>
    {
        logDebugMessage("state_result","5")

        return try {

            val response = apiService.getDriverChargeLogs(requestDriverRechargeLogs)
